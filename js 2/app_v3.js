...
// import { initAsistencia } from './components/asistencia.js'; // REDUNDANTE: Ya gestionado en app.js
...
// Detección inteligente del ID (Por parámetro ?id= o por carpeta)
const getEventIdFromUrl = () => {
    const params = new URLSearchParams(window.location.search);
    if (params.get('id')) return params.get('id');

    // Si no hay parámetro, miramos si estamos en una carpeta de cliente (ej: adriana-2026)
    const path = window.location.pathname;
    const parts = path.split('/').filter(p => p && !p.includes('.'));
    const lastFolder = parts[parts.length - 1];

    // Lista negra de carpetas de sistema para no confundirlas con IDs
    const systemFolders = ['apps', 'admin', 'modulos', 'js', 'assets', 'css', 'img'];

    if (lastFolder && !systemFolders.includes(lastFolder)) {
        return lastFolder;
    }
    return 'demo';
};
...
const cargarModulo = async (id, url) => {
    try {
        // Cache busting forzar recarga en servidores con caché agresivo
...
const hydrateConfigFromFirestore = async (eventId) => {
    try {
        ...
        // FALLBACK: Si no está publicado, buscar en registros temporales
        if (!eventSnap.exists()) {
            console.log("🔍 No encontrado en 'eventos', buscando en registros temporales...");
...
const applyHydration = (data) => {
    ...
    config.app.title = `${data.protagonistName} - Mi Primera Comunión`;

    // 2. Protagonista (Prioridad a campos nuevos, fallback a legacy)
    config.protagonista.nombre = data.protagonistName || config.protagonista.nombre;
    ...
    config.showMenu = data.showMenu !== false; // Default true if undefined
    ...
    if (nameEl && config.protagonista.nombre) {
        ...
        const adjustSize = () => {
            ...
            if (subtitleEl) {
                ...
                subtitleEl.style.margin = '1.5rem auto 2rem auto'; // Added top margin
            }
        };
...
let giftSettingsData = { iban: "", bizumPhone: "", beneficiary: "", bankName: "" };
...
window.loadSettings = () => {
    onSnapshot(doc(db, 'eventos', EVENT_ID, 'settings', 'configuracion'), snap => {
        if (snap.exists()) {
            ...
            giftSettingsData.notificationsVisible = c.notificationsVisible === true;
            giftSettingsData.notificationPhone = c.notificationPhone || "";

            // 2. UI de Invitados (Frontend)
            ...
            if (regaloSection) {
                const hasGiftOptions = (c.ibanVisible === true) || (c.bizumVisible === true) || (c.giftListVisible === true);
                regaloSection.classList.toggle('hidden', !hasGiftOptions);
                regaloSection.style.display = hasGiftOptions ? 'block' : 'none';
            }
            ...
            // 3. UI del Menú
            if (c.menuVisible !== undefined) config.showMenu = c.menuVisible;
            const menuContainer = document.getElementById('menu-selection-container');
            if (menuContainer) menuContainer.style.display = config.showMenu ? 'block' : 'none';

            // 4. SINCRONIZACIÓN PANEL ADMIN (FORCE BRUTE CSS)
            const togglesToSync = [
                { id: 'admin-menu-toggle', state: config.showMenu },
                { id: 'admin-iban-toggle', state: c.ibanVisible === true },
                { id: 'admin-bizum-toggle', state: c.bizumVisible === true },
                { id: 'admin-notifications-toggle', state: c.notificationsVisible === true },
                { id: 'admin-giftlist-toggle', state: c.giftListVisible === true }
            ];

            togglesToSync.forEach(ts => {
            ...
            // Re-vincular listener del menú
            window.initMenuToggleListener();

            // Sincronizar todos los inputs del admin
            const inputs = {
            ...
            for (const [id, val] of Object.entries(inputs)) {
                ...
                if (input) {
                    input.value = val || "";
                    if (id === 'admin-notification-phone' && val) {
                        input.dataset.confirmedValue = val;
                    }
                }
            }

            // Gestión de Secciones (IBAN/Bizum details)
            ['iban', 'bizum'].forEach(type => {
                const content = document.getElementById(`${type}-details-content`);
                if (content) content.classList.toggle('hidden', !c[`${type}Visible`]);
            });

            // Botón de cierre (Asegurar existencia)
...
window.toggleAdminSubSection = (type) => {
    // Esta función ahora solo aplica a IBAN y Bizum
    const content = document.getElementById(`${type}-details-content`);
...
/**
 * Alterna la visibilidad del menú de comida y guarda la preferencia.
 */
window.toggleMenuVisibility = async (ev) => {
    console.log("🖱️ toggleMenuVisibility Clicked! Current state:", config.showMenu);

    // Toggle state
    config.showMenu = !config.showMenu;
    console.log("➡️ New state:", config.showMenu);

    // Actualizar UI visualmente (Feedback inmediato)
    // Actualizar UI visualmente (Feedback inmediato + Fuerza Bruta CSS)
    let toggle = document.getElementById('admin-menu-toggle');
    if (typeof ev !== 'undefined' && ev && ev.currentTarget) {
        toggle = ev.currentTarget;
    }

    if (toggle) {
        // Force the class toggle based on the new boolean state
        const dot = toggle.querySelector('.toggle-dot');
        if (config.showMenu) {
            toggle.classList.add('active');
            toggle.style.backgroundColor = '#10b981'; // Green forced
            if (dot) dot.style.transform = 'translateX(24px)';
        } else {
            toggle.classList.remove('active');
            toggle.style.backgroundColor = '#374151'; // Gray forced
            if (dot) dot.style.transform = 'translateX(0px)';
        }
        console.log("🎨 Toggle UI updated FORCEFULLY. Class list:", toggle.classList);
    } else {
        console.error("❌ Element #admin-menu-toggle not found!");
    }

    const menuContainer = document.getElementById('menu-selection-container');
    if (menuContainer) {
        menuContainer.style.display = config.showMenu ? 'block' : 'none';
    }

    try {
        console.log("💾 Saving to Firestore...");
        await setDoc(doc(db, 'eventos', EVENT_ID, 'settings', 'configuracion'), {
            menuVisible: config.showMenu
        }, { merge: true });
        console.log("✅ Saved successfully.");
        // window.showAppToast(config.showMenu ? "Menú activado" : "Menú desactivado", "🍽️");
    } catch (error) {
        console.error("Error al guardar estado del menú:", error);
        window.showCustomAlert("Error al guardar la configuración.", "❌");
    }
};

// Force initialization of the toggle listener
window.initMenuToggleListener = () => {
    const toggle = document.getElementById('admin-menu-toggle');
    if (toggle) {
        // Remove old listeners to avoid duplicates (though difficult with anonymous functions, this is a safety net)
        toggle.onclick = window.toggleMenuVisibility;
        console.log("✅ Menu Toggle Listener Attatched via JS");
    }
};
...
window.enterPinDigit = (d) => {
    ...
    if (currentPin.length < 12) {
        ...
        const isMasterPin = enteredPin === "147" || enteredPin === "2580" || enteredPin === "147258";
...
window.confirmarDescuento = async () => {
    ...
    try {
        ...
        const adminEmail = 'apps@pujaltefotografia.es';
        const backupEmail = 'ganwich77@gmail.com';
        let recipients = [adminEmail, backupEmail];
        if (config.fotografo.email && !recipients.includes(config.fotografo.email)) {
        ...
        await addDoc(collection(db, 'mail'), {
            ...
            message: {
                ...
                html: `
                    <p>Hola,</p>
                    <p>Has recibido un nuevo registro a través del formulario de promoción <strong>"Un Regalo para Ti"</strong>. Aquí tienes los detalles del contacto para realizar el seguimiento:</p>
                    ...
                    <h3>📊 Detalles de la Conversión</h3>
                    <p>
                        <strong>Oferta:</strong> 10% de descuento en packs de sesiones.<br>
                        <strong>Estado de Privacidad:</strong> ✅ Aceptó la política de privacidad.<br>
                        <strong>Fecha/Hora:</strong> ${fechaActual}
                    </p>
                    <hr>
                    <small>Este es un mensaje automático de tu Asistente Digital.</small>
                `
            }
        });
