...
async function loadData() {
    ...
    try {
        ...
        [...pending, ...approved].forEach(({ id, data: p }) => {
            ...
            const pLogo = p.logo_url || p.logoUrl || 'assets/images/logo basecode.png';
            ...
            <div class="glass p-8 rounded-3xl flex flex-col items-center text-center group hover:border-emerald-500/50 transition duration-500 ${isPending ? 'border-amber-500/50 bg-amber-500/5' : 'cursor-pointer hover:bg-white/5'} relative overflow-hidden" ${!isPending ? `onclick="openEventsViewer('${id}', '${pName}', '${pLogo}')"` : ''}>
                ...
                <div class="w-24 h-24 bg-white/5 rounded-2xl flex items-center justify-center p-4 mb-4 backdrop-blur-sm group-hover:scale-110 transition duration-500">
                    <img src="${pLogo}" class="max-w-full max-h-full object-contain" onerror="this.src='assets/images/logo basecode.png'">
                </div>
...
window.openEventsViewer = async (photoId, photoName, photoLogo) => {
    ...
    document.getElementById('viewer-photo-logo').src = photoLogo || 'assets/images/logo basecode.png';
...
window.openGlobalAppsDirectory = async () => {
    ...
    document.getElementById('viewer-photo-logo').src = 'assets/images/logo basecode.png';
...
function renderViewerGrid(events) {
    ...
    grid.innerHTML = events.map(ev => {
        ...
        const f1 = photos[0] || ev.introFoto1 || ev.foto1 || 'assets/images/logo basecode.png';
        const f2 = photos[1] || ev.introFoto2 || ev.foto2 || 'assets/images/logo basecode.png';
        const f3 = photos[2] || ev.introFoto3 || ev.foto3 || 'assets/images/logo basecode.png';

        const eventUrl = `https://basecode.es/mi_primera_comunion/comuniones2026/${ev.id}/index.html?id=${ev.id}`;
        ...
        <div class="glass p-4 rounded-3xl flex flex-col gap-4 hover:border-emerald-500/30 transition group scale-up bg-slate-900/40">
            ...
            <div class="grid grid-cols-3 gap-2 h-32">
                <div class="col-span-2 h-full rounded-xl overflow-hidden bg-slate-800 border border-white/5">
                    <img src="${f1}" class="w-full h-full object-cover group-hover:scale-105 transition duration-700" onerror="this.src='assets/images/logo basecode.png'">
                </div>
                <div class="grid grid-rows-2 gap-2 h-full">
                    <div class="rounded-lg overflow-hidden bg-slate-800 border border-white/5">
                        <img src="${f2}" class="w-full h-full object-cover" onerror="this.src='assets/images/logo basecode.png'">
                    </div>
                    <div class="rounded-lg overflow-hidden bg-slate-800 border border-white/5">
                        <img src="${f3}" class="w-full h-full object-cover" onerror="this.src='assets/images/logo basecode.png'">
                    </div>
                </div>
            </div>
...
window.copyFormLink = (photoId) => {
    ...
    const link = `${baseUrl}datos_protagonista.html?f=${photoId}`;
                ...
                html: `
                    <div style="font-family: sans-serif; padding: 20px; color: #333; background-color: #f9f9f9; border-radius: 20px;">
                        ...
                        <div style="background: #000; color: #D4AF37; padding: 20px; border-radius: 15px; text-align: center;">
                            ...
                            <p style="margin-top: 20px;"><a href="https://basecode.es/mi_primera_comunion/acceso_fotografo.html" style="background: #D4AF37; color: black; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">ENTRAR AL PANEL</a></p>
                        </div>
                    </div>
                `
            }
