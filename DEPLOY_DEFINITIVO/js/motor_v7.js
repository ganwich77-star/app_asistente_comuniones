...
const hydrateConfigFromFirestore = async (eventId) => {
    try {
        ...
        if (eventDataForPhotog) {
            ...
            const fotografoId = eventDataForPhotog.photographerId || eventDataForPhotog.fotografo || eventDataForPhotog.fotografo_id || 'pujaltefotografia';
...
const populateStaticData = () => {
    ...
    setSafeText('ceremony-location', config.evento.ceremonia.lugar);

    setSafeText('banquet-name', config.evento.banquete.nombre);
    setSafeText('banquet-time', config.evento.banquete.hora);
    setSafeText('banquet-location', config.evento.banquete.lugar);
        ...
        if (snap.exists()) {
            ...
            if (regaloSection) {
                ...
                if (hasGiftOptions) {
                    ...
                    regaloSection.style.display = 'block'; // Asegurar visibilidad
                } else {
                    regaloSection.classList.add('hidden');
                    regaloSection.style.display = 'none';
                }
            }
                    ...
                    html: `
                        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0;">
                            ...
                            <div style="background-color: #f97316; padding: 32px; text-align: center;">
                                <h1 style="color: #ffffff; margin: 0; font-size: 24px; letter-spacing: 1px; text-transform: uppercase;">¡Nuevo Regalo Reclamado!</h1>
                                <p style="color: #ffedd5; margin-top: 8px; font-size: 14px;">Un invitado ha solicitado su cupón de descuento</p>
                            </div>
                            ...
                            <div style="padding: 32px; color: #334155;">
                                ...
                                <div style="text-align: center; margin-top: 32px;">
                                    <a href="https://apps.pujaltefotografia.es/admin/super_admin.html" style="background-color: #0f172a; color: #ffffff; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px; display: inline-block;">Ver en Super Admin</a>
                                </div>
                            </div>

                            <!-- Footer -->
                            <div style="background-color: #f1f5f9; padding: 24px; text-align: center; font-size: 12px; color: #64748b;">
                                <p style="margin: 0;">Este es un mensaje automático del Motor Maestro de Pujalte Creative Studio.</p>
                            </div>
                        </div>
                    `
                },
